...
function getClassName(props) {
  ...
  if (props.store.currentDate.getMonth() !== props.date.getMonth())
    return 'date inactive'
  else if (
    props.date.getDay() === WEEK_DAYS.Saturday ||
    props.date.getDay() === WEEK_DAYS.Sunday
  )
    className += ' weekend'

  if (checkEqualDates(props.date, props.store.today)) className += ' today'
  if (checkEqualDates(props.date, props.store.checkedDate))
    className += ' active'
