    ...
    return(
        <div className="select">
            <div className="select__top-wrapper">
                <input className="select__top__input"
                       placeholder={props.placeholder}
                       style={props.style}
                       value={chosenValue}
                       onChange={handleSelectInputChange}
                      />
                <img className="select__top__arrow"
                     src={ isSelectOpen ? upArrow : downArrow }
                     onClick={openSelectBottom}/>
            </div>

            <div className="select__bottom-wrapper"
                 style={{display: isSelectOpen ? "block" : "none"}}>
                { selectDetails.options.map((item) => {
                    return (
                        <div className="select__bottom__option"
                             key={item}
                             onClick={ () => handleSelectChooseOption(item) }>{item}</div> )
                })}
            </div>
        </div>
    )
